...
public class Jacobi implements Solver {
    ...
    public String solve(double[][] A, double[] y, String[] x, int p, double[] initGuess, int iterations, double epsilon) {
        ...
        if (A[A.length - 1][A[0].length - 1] == 0) {
            Functions.SwapRows(A, A.length - 1, A.length - 2);
            Functions.SwapValues(y, A.length - 1, A.length - 2);
        }
        try {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            for (int row = 0; row < n; row++) {
                ...
                for (int column = 0; column < n; column++) {
                    if (column != row) {
                        sum -= BigDecimal.valueOf(A[row][column] * prev[column]).setScale(p, RoundingMode.HALF_UP).doubleValue();
                    }
                }
    ...
    // checking error after each iteration to find if we reached the required accuracy if so then break
    // otherwise continue until we reach the required accuracy or we reached maximum number of iterations.
    public double calcE(double[] prev, double[] result) {
        double maxE = Double.MAX_VALUE * - 1;
        double temp;
        for (int i = 0; i < result.length; i++) {
            temp = Math.abs((result[i] - prev[i]) / result[i] * 100);
            if (temp > maxE) {
                maxE = temp;
            }
        }
